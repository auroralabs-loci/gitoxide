...
impl<'repo> Tree<'repo> {
    ...
    pub fn decode(&self) -> Result<gix_object::TreeRef<'_>, gix_object::decode::Error> {
        gix_object::TreeRef::from_bytes(&self.data, self.repo.object_hash().len_in_bytes())
    }
    ...
    pub fn find_entry(&self, name: impl PartialEq<BStr>) -> Option<EntryRef<'repo, '_>> {
        TreeRefIter::from_bytes(&self.data, self.id.kind().len_in_bytes())
        ...
        P: PartialEq<BStr>,
    {
        ...
        let mut data = gix_object::Data::new(gix_object::Kind::Tree, self.id.kind().len_in_bytes(), buf);
    ...
    {
        let mut iter = path.into_iter().peekable();
        let mut data = gix_object::Data::new(gix_object::Kind::Tree, self.id.kind().len_in_bytes(), &self.data);
...
mod iter {
    ...
    impl<'repo> Tree<'repo> {
        ...
        pub fn iter(&self) -> impl Iterator<Item = Result<EntryRef<'repo, '_>, gix_object::decode::Error>> {
            let repo = self.repo;
            gix_object::TreeRefIter::from_bytes(&self.data, self.id.kind().len_in_bytes())
