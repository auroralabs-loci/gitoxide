mod canonicalized {
    ...
    fn non_file_scheme_is_noop() -> crate::Result {
        let url = gix_url::parse("https://github.com/byron/gitoxide".into()).map_err(gix_error::Exn::into_error)?;
        assert_eq!(
    ...
    fn absolute_file_url_does_nothing() -> crate::Result {
        #[cfg(not(windows))]
        let url = gix_url::parse("/this/path/does/not/exist".into()).map_err(gix_error::Exn::into_error)?;
        #[cfg(windows)]
        let url = gix_url::parse(r"C:\non\existing".into()).map_err(gix_error::Exn::into_error)?;
        assert_eq!(
            url.canonicalized(&std::env::current_dir()?)
                .map_err(gix_error::Exn::into_error)?,
            url
    ...
    fn file_that_is_current_dir_is_absolutized() -> crate::Result {
        let url = gix_url::parse(".".into()).map_err(gix_error::Exn::into_error)?;
...
fn user() -> crate::Result {
    let mut url = gix_url::parse("https://user:password@host/path".into()).map_err(gix_error::Exn::into_error)?;
...
fn mutation_roundtrip() -> crate::Result {
    let mut url = gix_url::parse("https://user@host/path".into()).map_err(gix_error::Exn::into_error)?;
    url.set_user(Some("newuser".into()));
    url.set_password(Some("secret".into()));

    let serialized = url.to_bstring();
    let reparsed = gix_url::parse(serialized.as_ref()).map_err(gix_error::Exn::into_error)?;
...
fn from_bytes_roundtrip() -> crate::Result {
    ...
    let url = gix_url::parse(original.into()).map_err(gix_error::Exn::into_error)?;

    let bytes = url.to_bstring();
    let from_bytes = gix_url::Url::from_bytes(bytes.as_ref()).map_err(gix_error::Exn::into_error)?;
...
fn from_bytes_with_non_utf8_path() -> crate::Result {
    let url = gix_url::parse(b"/path/to\xff/repo".as_slice().into()).map_err(gix_error::Exn::into_error)?;
    let bytes = url.to_bstring();
    let from_bytes = gix_url::Url::from_bytes(bytes.as_ref()).map_err(gix_error::Exn::into_error)?;
...
fn user_argument_safety() -> crate::Result {
    let url = gix_url::parse("ssh://-Fconfigfile@foo/bar".into()).map_err(gix_error::Exn::into_error)?;
...
fn host_argument_safety() -> crate::Result {
    let url = gix_url::parse("ssh://-oProxyCommand=open$IFS-aCalculator/foo".into()).map_err(gix_error::Exn::into_error)?;
...
fn path_argument_safety() -> crate::Result {
    let url = gix_url::parse("ssh://foo/-oProxyCommand=open$IFS-aCalculator".into()).map_err(gix_error::Exn::into_error)?;
...
fn all_argument_safety_safe() -> crate::Result {
    let url = gix_url::parse("ssh://user.name@example.com/path/to/file".into()).map_err(gix_error::Exn::into_error)?;
...
fn all_argument_safety_not_safe() -> crate::Result {
    ...
    let url = gix_url::parse(all_bad.into()).map_err(gix_error::Exn::into_error)?;
