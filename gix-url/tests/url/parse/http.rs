...
fn empty_user_cannot_roundtrip() -> crate::Result {
    let actual = gix_url::parse("http://@example.com/~byron/hello".into()).map_err(gix_error::Exn::into_error)?;
...
fn username_and_empty_password() -> crate::Result {
    let actual = gix_url::parse("http://user:@example.com/~byron/hello".into()).map_err(gix_error::Exn::into_error)?;
...
fn percent_encoded_path() -> crate::Result {
    let url = gix_url::parse("https://example.com/path/with%20spaces/file".into()).map_err(gix_error::Exn::into_error)?;
...
fn percent_encoded_international_path() -> crate::Result {
    let url = gix_url::parse("https://example.com/caf%C3%A9".into()).map_err(gix_error::Exn::into_error)?;
